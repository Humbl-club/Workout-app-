    ...
    <ul className="space-y-2 text-[13px] text-[var(--text-secondary)] font-mono">
      {exercise.sets.map((set, index) => (
        <li key={index} className="flex justify-between items-center">
          <span className="text-[var(--text-tertiary)]">Set {set.set}</span>
          {'weight' in set ? (
            ...
            <span className="font-semibold">{set.duration_s}s</span>
          )}
        </li>
      ))}
    </ul>
    ...
    return (
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-5" style={{ boxShadow: 'var(--shadow-sm)' }}>
            <div className="border-b border-[var(--border)] pb-4 mb-4">
                ...
                <h3 className="font-syne text-xl font-bold text-[var(--text-primary)]">{log.focus}</h3>
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-5 pt-6 pb-[calc(5rem+env(safe-area-inset-bottom))] animate-fade-in flex-1">
      <header className="mb-8">
        <p className="text-[11px] uppercase tracking-wide text-[var(--text-tertiary)] font-semibold mb-2">TRAINING HISTORY</p>
        <h1 className="font-syne text-3xl font-bold text-[var(--text-primary)] leading-tight">Logbook</h1>
      </header>
        ...
        {Array.isArray(logs) && logs.length > 0 ? (
          logs.slice().sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(log => <LogCard key={log.id} log={log} />)
            ...
            <div className="text-center py-16 flex flex-col items-center justify-center bg-[var(--surface)] border border-[var(--border)] rounded-xl" style={{ boxShadow: 'var(--shadow-sm)' }}>
              ...
              <h3 className="font-syne text-xl font-bold text-[var(--text-primary)]">No Logs Yet</h3>
              <p className="mt-2 text-[14px] text-[var(--text-secondary)] max-w-xs px-4">Complete your first workout to see your history.</p>
          </div>
        )}
      </main>
    </div>
